...
// This returns two different concrete types depending on the argument, so it
// returns the existential `Shape`, not an opaque `some Shape` (which would
// have to be a single concrete type).
func makeShape(circular: Bool) -> Shape {
    if circular {
...
func main() {
    ...
    print("isSuccess \(result.isSuccess), square area \(makeShape(circular: false).area())")
