...
// Builds a row of elements (used by hstack).
@resultBuilder
...
// Stacks whole layouts on top of each other (used by the outer layout builder).
@resultBuilder
struct StackBuilder {
...
func hstack(alignment: Alignment = .left, @LayoutBuilder _ content: () -> [LayoutElement]) -> Layout {
    return Layout(elements: content(), alignment: alignment)
}
...
func main() {
    ...
    test("Menu with conditional items") {
        ...
        let firstSection = menu.items.first!
        if case .section(_, let items) = firstSection {
...
func Menu(@MenuBuilder _ content: () -> [MenuItem]) -> MenuType {
...
func buildForm(@FormBuilder _ content: () -> Form) -> Form {
    return content()
}

// Layout builder function
func buildLayout(@StackBuilder _ content: () -> [LayoutElement]) -> Layout {
    return Layout(elements: content(), alignment: .center)
}
